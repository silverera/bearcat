...
MetaUtil.resolveFuncAnnotation = function(func, fp, force) {
  ...
  if (funcString.match(/^\s*class/)) {
      funcArgsString = funcString.match(Constant.CLASS_ARGS_REGEXP);
  }

  if (funcArgsString) {
      funcArgsString = funcArgsString[1];
  } else {
      funcArgsString = funcString.match(Constant.FUNC_ARGS_REGEXP);
      if (funcArgsString) {
          funcArgsString = funcArgsString[1];
      }

      funcArgsString = funcArgsString ? funcArgsString : "";
  }
...
MetaUtil.resolvePropsFromFuncString = function(funcString, funcArgsString) {
  ...
  if (funcPropsArray && Utils.checkArray(funcPropsArray)) {
      for (var i = 0; i < funcPropsArray.length; i++) {
          t += (funcPropsArray[i].replace(/\s*super\.\$/, 'this.$') + EOL);
      }
  }
