...
ApplicationContext.prototype.prepareRefresh = function() {
  ...
  if (process.env.BEARCAT_HOT === 'on') {
      if (Utils.checkArray(hpath)) {
          for (var i = 0; i < hpath.length; i++) {
              this.hotReloadFileWatch(hpath[i]);
          }
      }
      else {
						...
						if (beanFunc) {
							...
							for (var beanName in extendBeanMap) {
								...
								for (var i = 0; i < superNames.length; i++) {
									...
									if (self.extendedBeanMap[cacheKey]) {
										delete self.extendedBeanMap[cacheKey];
									}
									self.doExtendBean(beanName, superBeanName);
								}
							}
...
ApplicationContext.prototype.extendBean = function(beanName, superBeanName) {
	...
	if(!this.extendBeanMap[beanName].includes(superBeanName))
...
ApplicationContext.prototype.doExtendBean = function(beanName, superBeanName) {
	...
	for (let name of Utils.getAllFuncs(_super)) {
