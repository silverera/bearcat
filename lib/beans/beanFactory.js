...
BeanFactory.prototype.createBean = function(beanName) {
	...
	beanObject = this.initBean(beanObject, beanName, this.getInitCb(beanName), args);
...
BeanFactory.prototype.doCreateBean = function(beanName) {
	...
	if (Utils.isClass(func)) {
		beanObject = new func(...dependsApplyArgs);
	} else {
		beanObject = Object.create(func.prototype);
 ...
 * @api private
 */
BeanFactory.prototype.initBean = function(beanObject, beanName, cb, args) {
	var beanDefinition = this.getBeanDefinition(beanName);
	if (!beanDefinition) {
		return;
	}
	...
	this.invokeInitMethods(beanObject, beanName, cb, args);
 ...
 * @api private
 */
BeanFactory.prototype.invokeInitMethods = function(beanObject, beanName, cb, args) {
	var beanDefinition = this.getBeanDefinition(beanName);
	if (!beanDefinition) {
		return;
	}
	...
	if (beanDefinition.hasInitMethod()) {
		...
		if (Utils.checkFunction(initMethod)) {
			...
			initMethod.apply(beanObject, args.concat(cb));
		}
	}
}
